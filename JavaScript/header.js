...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // تابع کمکی برای تبدیل قیمت رشته‌ای (مثل "3.200.000 تومان") به عدد
    const parsePrice = (priceStr) => {
        if (!priceStr) return 0;
        return parseInt(priceStr.replace(/\./g, '').replace(' تومان', '').trim(), 10);
    };

    // تابع کمکی برای فرمت کردن عدد به سبک ایرانی (با نقطه جداکننده هزارگان)
    ...
    const allProductCard = (product) => {
        const allCardItem = document.createElement("div");
        allCardItem.className = "cardItem cardShapeb";


        // محاسبه قیمت جدید و درصد تخفیف
        let discountBadge = "";
        let newPriceHTML = "";
        let oldPriceClass = "";
    ...
    const createAlternativeProductCard = (product) => {
        ...
        cardItem2.className = "cardItem cardShape";
    ...
    fetch('./products.json')
        ...
        .then(products => {
            ...
            const allProduct = products.filter(p => p.categories?.includes('all'));

            function renderAllProductSection(productList, wrapperId, btnId) {
                ...
                initial.forEach(product => wrapper.appendChild(allProductCard(product)));
                ...
                if (limited.length > 5) {
                    ...
                    btn.onclick = () => {
                        limited.slice(5).forEach(product => wrapper.appendChild(allProductCard(product)));
                        addLikeFunctionality();
                        convertNode(wrapper);
                        btn.style.display = 'none';
                    };
            ...
            renderAllProductSection(allProduct, '#all-product-wrapper', 'all-product-show-more');
            ...
            function renderAlternativeSection(productList, wrapperId, btnId) {
                const wrapper = document.querySelector(wrapperId);
                const btn = document.getElementById(btnId);
                ...
                initial.forEach(product => wrapper.appendChild(createAlternativeProductCard(product)));
                addLikeFunctionality();
                convertNode(wrapper);

                if (limited.length > 5 && btn) {
                    btn.style.display = 'block';
                    btn.onclick = () => {
                        limited.slice(5).forEach(product => wrapper.appendChild(createAlternativeProductCard(product)));
                        addLikeFunctionality();  // دوباره فراخوانی برای کارت‌های جدید
                        convertNode(wrapper);
                        btn.style.display = 'none';
                    };
                } else if (btn) {
            ...
            renderAlternativeSection(amazingOffers, '#amazing-offers-wrapper', 'amazing-show-more');
